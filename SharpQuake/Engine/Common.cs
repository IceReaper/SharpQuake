...
namespace SharpQuake.Engine
{
    ...
    public class Common
    {
        public GameKind GameKind => this._GameKind;

        public bool IsRegistered => this.MainWindow.Host.Cvars.Registered.Get<bool>( );
        ...
        public void Initialise( MainWindow mainWindow, string path, string[] argv )
        {
            ...
            this.MainWindow.Host.Commands.Add( "path", FileSystem.Path_f );

            CommandLine.Init( path, argv );
            FileSystem.InitFileSystem(this.MainWindow.Host.Parameters );
        ...
        public void InitArgv( string[] argv )
        {
            ...
            this._GameKind = GameKind.StandardQuake;

            if ( CommandLine.HasParam( "-rogue" ) )
                this._GameKind = GameKind.Rogue;

            if ( CommandLine.HasParam( "-hipnotic" ) )
                this._GameKind = GameKind.Hipnotic;
        }
        ...
        private void CheckRegistered( )
        {
            ...
            {
                this.MainWindow.Host.Console.Print( "Playing shareware version.\n" );
            ...
            for ( var i = 0; i < 128; i++ )
            {
                if ( Common._Pop[i] != ( ushort ) EndianHelper.Converter.BigShort( ( short ) check[i] ) )
                    Utilities.Error( "Corrupted data file." );
            }

            this.MainWindow.Host.CVars.Set( "cmdline", CommandLine._Args );
            this.MainWindow.Host.CVars.Set( "registered", true );
            FileSystem._StaticRegistered = true;
            this.MainWindow.Host.Console.Print( "Playing registered version.\n" );
        }
    }
}
